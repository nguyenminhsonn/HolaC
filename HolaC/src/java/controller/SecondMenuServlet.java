...
public class SecondMenuServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SecondMenuServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SecondMenuServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//        processRequest(request, response);
        ...
        List<Product> raw_listP = pDAO.getProductBySid(brand);
        request.setAttribute("dataC", listC);
        request.setAttribute("dataB", listB);
        request.setAttribute("dataP", raw_listP);
